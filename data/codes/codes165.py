...
def adv_attack(model, X, y, device):
    ...
    # X_original = X_adv
    # for time in range(K):
    #     X_n = X_adv.data.numpy()
    #     grad_n = np.gradient(X_n)
    #     grad_n = np.array(grad_n)
    #     grad_n = w1 * grad_n[0] + (1 - w1) * grad_n[1]
    #     grad_t = torch.from_numpy(grad_n)
    #     sign = grad_t.sign()
    #     r = epsilon * sign
    #     X_adv = Variable(X_adv + alpha * r)
    # X_adv = Variable((X_original.data + r))


    ## art
    with torch.enable_grad():
        adver_example = projected_gradient_descent(model, X_adv, y=y, eps=0.1, eps_iter=0.08, nb_iter=8, norm=np.inf)
        X_adv = Variable(adver_example)
...
def train_model():
    ...
    optimizer = optim.Adamax(model.parameters(), lr=0.005, betas=(0.9, 0.999))
