...
class TemporaryEnableGrad(object):
    def __enter__(self):
...
def adv_attack(model, X, y, device):
    ...
    with TemporaryEnableGrad():
        model_cp = copy.deepcopy(model)
        X_adv = Variable(X_adv.reshape(-1, 28*28).to(device), requires_grad=True)
        labels = Variable(y.to(device))

        outputs = model_cp(X_adv)
        ...
        X_adv = torch.clamp(X_adv.data + 0.1099 * grad, 0, 1)
