...
def adv_attack(model, X, y, device):
    with torch.enable_grad():
        ...
        model.zero_grad()
        F.nll_loss(model(X_adv), y).backward()
        r_adv = X_adv + 0.1 * X_adv.grad / torch.norm(X_adv.grad)
        model.zero_grad()
...
def train(args, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        F.nll_loss(model(adv_data), target).backward()
...
# load the model
# model = Net().to(device)
# model.load_state_dict(torch.load(str(id_) + '.pt'))
# model = train_model()
