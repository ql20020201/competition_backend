...
def adv_attack(model, X, y, device):
    ...
    for i in range(iters):
        ...
        criterion = Variable(cost, requires_grad=True)
        criterion.backward()

        adv_images = Variable(images.data +  alpha, requires_grad=True)
        model.zero_grad()
        eta = torch.clamp(adv_images - ori_images, min=-eps, max=eps)
        images = torch.clamp(ori_images + eta, min=0, max=1).detach_()

    random_noise = torch.FloatTensor(*images.shape).uniform_(-0.1, 0.1).to(device)
    X_adv = Variable(images.data + random_noise, requires_grad=True)
    return X_adv
