...
### 2. an adversarial training algorithmp
###
...
def adv_attack(model, X, y, device):
    ...
    # Step-3:zero grad
    model.zero_grad()
    # Step-4:backward loss
    loss.requires_grad_(True)
    loss.backward()
    
    # return test data
    if X_adv.grad is None:
        return Variable(X_adv.data + torch.FloatTensor(*X_adv.shape).uniform_(-0.1, 0.1).to(device))
    
    # Antagonism sample generation
    X_adv = Antagonism_sample_generation(X_adv, X_adv.grad.data)
...
def Antagonism_sample_generation(arr, grad, epslion=0.1):
    sign_grad = grad.sign()
    nor = arr + epslion * sign_grad
    return torch.clamp(nor, 0, 1)
...
def train_model():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print('Epoch:【{}/{}】| cost-time:{}s | train_loss:{}, train_acc:{:.2f}% | adv_loss:{}, adv_acc:{:.2f}% |'.format(str(epoch), 
        args.epochs, str(int(time.time()-start_time)), 
        trnloss, 100. * trnacc,
        advloss, 100. * advacc
        ))
