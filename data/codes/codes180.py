...
def adv_attack(model, X, y, device):
    ...
    random_noise = torch.FloatTensor(*X_adv.shape).uniform_(-0.1094, 0.1094).to(device)
    X_adv=Variable(X_adv.data+random_noise, requires_grad=True)
    ab=optim.SGD([X_adv], lr=0.25)
    ab.zero_grad()
    with torch.enable_grad():
        loss=nn.CrossEntropyLoss()(model(X_adv), y)
    loss.backward()
    cd=0.1094*X_adv.grad.data.sign()
    X_adv=Variable(X_adv.data+cd, requires_grad=True)
    cd=torch.clamp(X_adv.data-X.data, -0.1094, 0.1094)
    X_adv=Variable(X.data+cd, requires_grad=True)
