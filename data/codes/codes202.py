...
def adv_attack(model, X, y, device):
    ...
    ################################################################################################
    # The idea:
    # A take on the black box Square attack:
    # From the paper ‘Square Attack: a query-efficient black-box adversarial attack via random search’
    # [https://arxiv.org/abs/1912.00049]

    # searching through tensors, finding the bounds of pixels with values != 0, and creating semi random noise in a
    # square x,y in a random direction near the value

    # For non targeted attacks, works best in column streaks of width 1

    random_noise = torch.FloatTensor(*X_adv.shape).uniform_(-0.1, 0.1).to(device)

    model.eval()

    # Convert the images and labels to tensors and add batch dimensions
    X = X.to(device)
    y = y.to(device)
    ...
    # Set the alpha parameter for the Square Attack
    alpha = 0.1

    # Set the initial perturbations to zero
    # perturbations = torch.zeros_like(X)
    perturbations = torch.rand_like(X) * 0.105

    # Set the step size for the search
    step_size = 0.01


    # Set the desired adversarial label
    adversarial_label = 0

    # Iterate over the number of maximum iterations
    for i in range(max_iter):
        # Calculate the model output and decision boundaries
        output = model(X + perturbations)
        # decision_boundaries = (output[:, adversarial_label] - output[:, y]) / 2
        decision_boundaries = (alpha * torch.norm(perturbations, dim=1) - output[:, adversarial_label]) / 2
        # Calculate the loss
        loss = decision_boundaries - alpha * torch.norm(perturbations, dim=1)


        # Check if the loss is below zero for any image in the batch
        #if (loss < 0).any():
        if torch.mean(loss).item() < -0.1:

            # If the loss is below zero, we have found an adversarial image
            # so we can break the loop
            break


        # If the loss is not below zero, we have not found an adversarial image
        # so we update the perturbations and continue the search
        perturbations = (perturbations + step_size)# * torch.sign(loss) # 0.105 p,  attack 1.3594
        # perturbations = (perturbations + step_size) * torch.sign(loss)
        step_size = step_size*1.1
        # Clip the perturbations to the range [-0.5, 0.5]
        perturbations = torch.clamp(perturbations, -0.1099, 0.1099)


    # The final adversarial images are the original images plus the perturbations
