...
def adv_attack(model, X, y, device):
    ...
    X_adv = X_adv.view(X.size(0), 28 * 28)
    ...
    random_noise = eps * sign_data_grad
    X = torch.clamp(Variable(X_adv.data + random_noise), 0, 1)
...
def train(args, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
         ...
         sign_data_grad = data_grad.sign()
         random_noise = eps * sign_data_grad
         X_adv = torch.clamp(Variable(data.data + random_noise), 0, 1)
         loss = F.nll_loss(model(X_adv), target)
